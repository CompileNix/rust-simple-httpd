...
impl HttpServer {
    ...
    async fn read_stream_into(
        ...
        let bytes_read = match result_read {
            ...
            Err(err) => {
                warn!(cfg, "Worker: 🤷 Error while reading from remote, aborting connection. Here is the error we got: {err}");
                return Err(anyhow!(err));
            }
        };
        ...
        trace!(
            cfg,
            "stream.read(buffer({len})): returned {bytes_read} bytes",
            len = buffer.len()
        );
    ...
    pub async fn handle_connection(&self, stream: &mut TcpStream) {
        ...
        debug!(cfg, "begin reading stream into buffer until EOH");
        trace!(
            cfg,
            r#"EOH sequence is "\r\n\r\n" (0x0d, 0x0a, 0x0d, 0x0a)"#
        );
        loop {
            ...
            let Some(buffer_trimmed) = buffer.get(..bytes_read) else {
                warn!(cfg, "We could not trim the input buffer. The buffer has a length of {buffer_size} and we got {bytes_read} bytes.", buffer_size = self.config.buffer_client_receive_size);
                return;
            };
            ...
            if let Some(eoh_byte_index) = Self::bytes_contain_eoh(request_data.as_slice()) {
                trace!(cfg, "EOH sequence found at {eoh_byte_index}");
        ...
        if request_header_limit_bytes_exceeded {
            warn!(cfg, "Request header size limit of {request_header_limit_bytes} has been exceeded. Aborting connection...", request_header_limit_bytes = self.config.request_header_limit_bytes);
            self.send_basic_status_response(stream, 400, "Bad Request")
                .await;
            return;
        }
